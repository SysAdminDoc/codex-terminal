...
[![Version](https://img.shields.io/badge/version-0.12.0-cba6f7?style=flat-square)](CHANGELOG.md)
...
| `codexTerminal.notifyOnCompletion` | `false` | Opt in to turn-completion notifications without modifying `~/.codex/config.toml`; machine-scoped, and the temporary hand-off stores only event metadata. |
...
rollouts there are 198 `task_started` events, 108 `task_complete` events, and 29 `turn_aborted`
events; 61 turns have no terminal record at all. A `turn_aborted` record is detected and shown as
**Interrupted**. A turn with no terminal record can remain claiming to be working until the
**Silent** threshold, which is why silence is treated as uncertainty rather than completion.

A session that has written nothing for far longer than any real turn goes quiet and is marked
...
```powershell
codium --install-extension codex-terminal-0.12.0.vsix   # VSCodium
code   --install-extension codex-terminal-0.12.0.vsix   # VS Code
```
...
npm run package   # -> dist/codex-terminal-0.12.0.vsix
```
...
npm ci
npm run package
Get-FileHash -Algorithm SHA256 dist\codex-terminal-0.12.0.vsix
```
